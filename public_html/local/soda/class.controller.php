...
class controller {
    ...
    var $auto_replace_vars = false; // replaces repeatcourse variables like {$username}, {$city}, {$user.firstname}
    var $plugin_type;
     ...
     * @param  string $repeatcourse   Whether to use the module's repeatcourse, defaults to true (optional)
     * @return void
     */
    function get_view($data_array = array(), $view = false, $repeatcourse = true) {
        global $CFG, $id;
        ...
        $repeatcourse_path = "{$CFG->dirroot}/{$this->plugin_type}/$this->mod_name/views/repeatcourse.html";
        if (file_exists($repeatcourse_path)) {
            include_once($repeatcourse_path);
            return;
        }
