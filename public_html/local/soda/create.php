...
if ( shell_exec( " mkdir $dir_dest 2>&1") == null ) {
    echo shell_exec(" cp -r -a repeatcourse/* $dir_dest 2>&1 " );
    echo shell_exec(" find $dir_dest -iname '*repeatcourse*' -exec rename 's/repeatcourse/$mod_name/i' {} + ");
    shell_exec(" find $dir_dest -exec sed -i -e 's/repeatcourse/$mod_name/g' {} \; 2>&1 ");
    shell_exec(" find $dir_dest -exec sed -i -e 's/repeatcourse/" . ucfirst($mod_name) . "/g' {} \; 2>&1 ");
...
if ( !empty($parsed_args['options']['skeleton']) ) {
    ...
    foreach($skeleton_dirs as $dir) {
        ...
        shell_exec(" cp -r -a repeatcourse/$dir/* $dir_dest 2>&1 " );
    }
    foreach($skeleton_files as $dir) {
        $file_dest = "../../mod/$mod_name/$dir/$model_name.php";
        shell_exec(" cp -a repeatcourse/$dir/repeatcourse.php $file_dest 2>&1 " );
    }
    $dir_dest = "../../mod/$mod_name";
    shell_exec(" find $dir_dest -exec sed -i -e 's/repeatcourse/$model_name/g' {} \; 2>&1 ");
    shell_exec(" find $dir_dest -exec sed -i -e 's/repeatcourse/" . ucfirst($model_name) . "/g' {} \; 2>&1 ");
}
?>
