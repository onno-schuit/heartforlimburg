    ...
    foreach($repCourses as $repcourse){
        ...
        foreach($repeatCourseRecords as $repeatCourseRecord){
			...
			//if(($endDateArr['mday'] == $curDateArr['mday'] && $endDateArr['mon'] == $curDateArr['mon'] && $endDateArr['year'] == $curDateArr['year'])){
                ...
                $curCourseUsers = $DB->get_records_sql('SELECT cc.userid, cc.timecompleted FROM {course_completions} AS cc INNER JOIN {course_completions_repcourse} AS ccr ON cc.userid = ccr.userid WHERE ccr.maincourseid = cc.course AND cc.course=' . $repeatCourseRecord->maincourseid . ' AND ccr.mailing = 1 AND cc.timecompleted IS NOT NULL');
				
                foreach($curCourseUsers as $ccu){
					//-------------------------------------------------------------------------------------------
					// Current day
					//-------------------------------------------------------------------------------------------
					$endDateArr = getdate(intval($ccu->timecompleted) + (intval($repeatCourseRecord->cinterval)*86400));
					...
					//-------------------------------------------------------------------------------------------
					// Reminder before course starts depending on config
					//-------------------------------------------------------------------------------------------
	   				$startDateReminder = intval($ccu->timecompleted) + ((intval($repeatCourseRecord->cinterval) - intval(get_config('repeatcourse', 'daytoremind')))*86400);
					$startDateCourse = intval($ccu->timecompleted) + (intval($repeatCourseRecord->cinterval)*86400);
